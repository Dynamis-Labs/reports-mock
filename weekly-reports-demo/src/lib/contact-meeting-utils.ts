...
/**
 * Find the most recent meeting where a contact was an attendee.
 * Matches by linkedContactId, email, or full name.
 */
export function findMostRecentMeeting(
  contact: Contact,
  meetings: Meeting[],
): Meeting | null {
  ...
  const contactMeetings = meetings.filter((meeting) =>
    meeting.attendees.some((att) => {
      // Match by linkedContactId
      if (att.linkedContactId === contact.id) return true;
      // Match by email
      if (att.email.toLowerCase() === contactEmail) return true;
      // Match by name
      if (att.name.toLowerCase() === contactFullName) return true;
      return false;
    }),
  );
  ...
  return contactMeetings.sort((a, b) => b.date.getTime() - a.date.getTime())[0];
}
